...
from app.models import CustomUser, Student, Staff, Subject
    ...
    if request.method == "POST":
        ...
        if CustomUser.objects.filter(email=email).exists():
            ...
            messages.success(request, user.first_name+" " +
                             user.last_name+" "+"Sucessfully Added")
            return redirect('add_student')
...
def ADD_COURSE(request):
    if request.method == "POST":
        ...
        course.save()
        messages.success(request, "Course Succcessfully Added")
...
def ADD_STAFF(request):
    if request.method == "POST":
        ...
        # if CustomUser.objects.filter(email=email).exists:
        #     messages.warning(request, "Email Already Eists!!")
        #     return redirect('add_staff')
        # if CustomUser.objects.filter(username=username).exists:
        #     messages.warning(request, "Username Already Eists!!")
        #     return redirect('add_staff')
        # else:
        user = CustomUser(
            first_name=first_name,
...
def DELETE_STAFF(request, admin):
    ...
    staff.delete()
    messages.success(request, "Staff Details Deleted Succuessfully")
    return redirect('view_staff')
...
def STAFF_SEND_NOTOFICATION(request):
